...
while True:
    ...
    for (top, right, bottom, left), face_encoding in zip(face_locations, face_encodings):
        ...
        for index, result in enumerate(results):
            if result:
                person = known_names[index]
                print("I see you " + person)
            if capture_interval == 0 and song_playing != person:
                os.system("osascript -e 'tell application \"spotify\" to play track \"spotify:track:" + spotify_tracks[person] + "\"'")
                if person == "Unknown":
